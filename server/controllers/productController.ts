...
export const getProductswithId = async (req: Request, res: Response) => {
  ...
  if (!product) {
    res
      .status(404)
      .json({ message: "There is no product present with this id." });
    return;
  }
  ...
  res.json({ product: { ...product, discountPrice } });
};
